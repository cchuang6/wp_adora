...
$html.= '<div id="opts-'.$section_name.'-'.$css_selector.'-'.$property_group_name.'-'.$property. $mq_extr_class. '"
 class="property-tag field-wrap tvr-clearfix '.$man_class . ' '.$field_class . ' field-'.$property_group_name.'-'.$property.'">
    ...
    <span class="comp-mixed-table">
		<h5 title="' . esc_attr__('Hover over the rows to highlight the individual elements on the page', 'tvr-microthemer') . '">' . esc_html__('Page Analysis', 'tvr-microthemer') . '</h5>
        <table>
            <thead>
            <tr>
                <th><i>n</i></th>
				<th>' . esc_html__('Value', 'tvr-microthemer') . '</th>
            </tr>
            </thead>
            <tbody></tbody>
        </table>
