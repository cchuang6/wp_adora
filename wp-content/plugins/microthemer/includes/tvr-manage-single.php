...
<div id="tvr" class='wrap tvr-wrap tvr-manage tvr-manage-single'>
	...
	<div id='tvr-manage-single'>
		...
		<p><a class="no-line" href="<?php echo 'admin.php?page='. $this->microthemespage;?>"><?php esc_html_e("&laquo; Back to all packs", "tvr-microthemer");?></a></p>
        ...
        <div id='pack-summary'>
            ...
            <div class="summary-text">
                ...
                if (!empty($meta_info['Author'])) {
                    $h2.= '<p class="author-name">' . sprintf( esc_html__( 'by %s', 'tvr-microthemer'), strip_tags($meta_info['Author']) ) . '</p>';
                } else {
                    $h2.= '<br />';
                }
                echo $h2;
                if ($meta_info['Description']){
                    ?>
					<div class="heading"><?php esc_html_e('Description', 'tvr-microthemer'); ?></div>
                    <p><?php echo $meta_info['Description']; ?></p>
                    <?php
                } else {
                    ?>
					<div class="heading"><?php esc_html_e('No Description Available', 'tvr-microthemer'); ?></div>
					<p><?php esc_html_e('Optionally enter a description using the "Info" form below.', 'tvr-microthemer'); ?></p>
                    <?php
                }

                if (count($meta_info['Tags']) > 0) {
                    ?>
					<div class="heading"><?php esc_html_e('Tags', 'tvr-microthemer'); ?></div>
        ...
        <div id="single-content-areas">
            <div id='edit-info' class='manage-content-wrap hidden show'>
                <div class="explain">
					<div class="heading"><?php esc_html_e('About This Feature', 'tvr-microthemer'); ?></div>
					<p><?php printf(
						esc_html__('If you plan to make your design pack available for sale or free download on themeover.com, please fill out all of the form fields on the right. Hover your mouse over the field labels to view instructions for each field. You will also need to upload a thumbnail image on the %1$s and (optionally) some instructions for the end user on the %2$s', 'tvr-microthemer'),
							'<span class="pack-action link" rel="attachments">' . esc_html__('Attachments tab', 'tvr-microthemer') . '</span>',
							'<span class="pack-action link" rel="instructions">' . esc_html__('Instructions tab', 'tvr-microthemer') . '</span>'
					    ); ?></p>
					<p><?php esc_html__('The information you enter will update the meta.txt files in your design pack. Themeover will make use of this information if you submit your design pack for inclusion in our marketplace.', 'tvr-microthemer'); ?></p>
					<p><?php printf(
						esc_html__('It\'s not necessary to fill out these fields if don\'t wish to share your design pack. But you may wish to upload a thumbnail image to replace the "No Screenshot" placeholder on the %s.', 'tvr-microthemer'),
							'<span class="pack-action link" rel="attachments">' . esc_html__('Attachments tab', 'tvr-microthemer') . '</span>'
					); ?></p>
                </div>
				<div class="heading"><?php esc_html_e('Design Pack Meta Info', 'tvr-microthemer'); ?></div>
                <form name='edit_meta_form' id="edit-meta-form" method="post" class='float-form' autocomplete="off"
                    ...
                    <p class="combobox-wrap input-wrap">
						<label><?php esc_html_e('Type of Design Pack: ', 'tvr-microthemer'); ?></label>
					...
					<p><label><?php esc_html_e('Design Pack Name: ', 'tvr-microthemer'); ?></label>
            ...
            <div id='edit-attachments' class='manage-content-wrap hidden'>
                <div class="explain">
                    <div class="heading"><?php esc_html_e('About This Feature', 'tvr-microthemer'); ?></div>
					<p><?php esc_html_e('Upload a screenshot image file 896px wide and 513px tall called "screenshot.gif/jpg/png" to give your design pack a nice thumbnail. It\'s important to upload a full-size thumbnail in the dimensions 896 x 513 or larger. Microthemer will crop your image to 896 x 513 if you upload a larger image. It will also automatically create a smaller version called screenshot-small.gif/jpg/png.','tvr-microthemer'); ?></p>
					<p><?php printf(wp_kses(__('<b>Important Note:</b> As of version 3 Microthemer now uses the WordPress media manager for storing background images. It\'s  therefore not necessary or advised to upload background images here. It\'s much better to upload them from the Microthemer UI page, when you select a background image for inclusion in your design.', 'tvr-microthemer'), array( 'b' => array() ))
						); ?></p>
					<p><?php esc_html_e('Any images your design pack links to will be included in your design pack if you choose to download it as a zip file. When you install a design pack zip file, all the images will be copied to the WordPress media library. Image file paths will be updated accordingly.', 'tvr-microthemer'); ?></p>
                </div>
				<div class="heading"><?php esc_html_e('Design Pack Files', 'tvr-microthemer'); ?></div>
                            ...
                            if ($array['location'] == 'library' and !$library){
                                ...
                                <tr>
									<td scope="col" colspan="3" class="library-heading heading"><?php
										esc_html_e('Media library images this design pack incorporates', 'tvr-microthemer');
									?></td>
                               </tr>
                                <?php
                            }
                ...
                <div class='file-types'>
					<p><?php esc_html_e('File types you are allowed to upload:', 'tvr-microthemer'); ?></p>
                    <?php
            ...
            <div id='edit-instructions' class='manage-content-wrap hidden'>
                <div class="explain">
					<div class="heading"><?php esc_html_e('About This Feature', 'tvr-microthemer'); ?></div>
					<p><?php esc_html_e('If you\'re going to submit your design pack to Themeover, adding some instructions to help end users customize your design pack is a really nice touch. This could include pointing out potential pitfalls or recommending best practices. The instructions you add will feature on Themeover\'s listing page.', 'tvr-microthemer'); ?></p>
					<p><?php esc_html_e('Some design packs may not benefit much from additional instructions of course. In which case feel free to leave this blank.', 'tvr-microthemer'); ?></p>
                </div>
				<div class="heading"><?php esc_html_e('Instructions For The End User', 'tvr-microthemer'); ?></div>
                ...
                <form name='edit_readme_form' id="edit-readme-form" method="post" class='float-form' autocomplete="off"
                    ...
                    <p class="button-wrap">
						<input class="tvr-button" type="submit" name="tvr_edit_readme_submit" value="<?php esc_attr_e('Update Instructions', 'tvr-microthemer'); ?>" />
                    </p>
                </form>
            </div>
        ...
        <!-- View file dialog -->
        <?php echo $this->start_dialog('view-pack-file', esc_html__('Design Pack File', 'tvr-microthemer'), 'sidebar'); ?>
        <div class="explain">
			<div class="heading"><?php esc_html_e('About this file', 'tvr-microthemer'); ?></div>
            <div class="explain-config">
				<p><?php esc_html_e('This is the configuration file which contains all of the Microthemer settings for this design pack. It is created automatically when you export your work. When you import a design pack into the Microthemer UI the style settings, media queries, and any custom CSS code are loaded from this file.', 'tvr-microthemer'); ?></p>
				<p><?php esc_html_e('You\'re not likely to want to manually edit this file. But occasionally programmers download it, do some find and replace adjustments, and then re-upload it using the "Upload File" button at the top of the design pack files table.', 'tvr-microthemer'); ?></p>
            </div>
            <div class="explain-meta">
				<p><?php esc_html_e('This is the information file which contains meta information about design pack. Themeover uses the information in this file to list a user-submitted design pack on themeover.com. You can edit this file using the "Design Pack Meta Info" form.', 'tvr-microthemer'); ?></p>
            </div>
            <div class="explain-instructions">
				<p><?php esc_html_e('This is the readme file which contains instructions for the end user of a design pack - if the author decided that instructions would be beneficial. You can edit this file using the "Instructions" form.', 'tvr-microthemer'); ?></p>
            </div>
        </div>
