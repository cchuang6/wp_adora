...
// if already defined, tell user to disable microloader
	...
	add_action(
		...
		create_function(
			'',
			'echo \'<div id="message" class="error"><p><strong>'.esc_html__('Microthemer requires that you deactivate Microloader. Please do so.', 'tvr-microthemer').'</strong></p></div>\';'
		)
	);
		...
		class tvr_microthemer_admin {
			...
			*/
			function __construct(){
				...
				if ( is_admin() and in_array($page, $this->all_pages) ) {
                    ...
                    $this->css_unit_sets = array(
                        ...
                        __('Auto-conversions', 'tvr-microthemer'),
                    );
                    ...
                    $this->css_units = array(
                        ...
                        'vw' => array(
                            ...
                            'desc' => __('&#37; of viewport\'s width', 'tvr-microthemer'),
                        ...
                        'vmin' => array(
                            'type' => 'viewport relative',
                            'desc' => __('&#37; of viewport\'s smaller dimension', 'tvr-microthemer'),
                            'not_supported_in' => array('IE8')
                            // NOTE: in IE9 it's called 'vm'
                        ),
                        'vmax' => array(
                            'type' => 'viewport relative',
                            'desc' => __('&#37; of viewport\'s larger dimension', 'tvr-microthemer'),
                            'not_supported_in' => array('IE8', 'IE9', 'IE10', 'IE11')
                        ),
                        'px to em'=> array(
                            'type' => 'auto-conversion',
                            'desc' => __('Enter a number of pixels (with no unit) and Microthemer will automatically convert the value to an equivalent em value.', 'tvr-microthemer')
                        ),
                        'px to rem'=> array(
                            'type' => 'auto-conversion',
                            'desc' => __('Enter a number of pixels (with no unit) and Microthemer will automatically convert the value to an equivalent rem value.', 'tvr-microthemer')
                        ),
                        'px to %'=> array(
                            'type' => 'auto-conversion',
                            'desc' => __('Enter a number of pixels (with no unit) and Microthemer will automatically convert the value to an equivalent percentage value.', 'tvr-microthemer')
                        )
                    );
					...
					add_action(
						...
						create_function(
							...
							'echo \'<div id="message" class="error"><p><strong>' .
								esc_html__('Sorry, Microthemer only runs on WordPress version %s or above. Deactivate Microthemer to remove this message.', 'tvr-microthemer') .
							'</strong></p></div>\';'
						)
					);
					return false;
				...
				if ( ($this->memory_limit != 0) and ($this->memory_limit < 16 ) ) {
					add_action(
						...
						create_function(
							'',
							'echo \'<div id="message" class="error"><p><strong>' .
								esc_html__('Sorry, Microthemer requires a memory limit of 16MB or higher to run. Your memory limit is less than this. Deactivate Microthemer to remove this message.', 'tvr-microthemer') .
							'</strong></p></div>\';'
						)
					);
					return false;
				...
				if ( $this->trial === false ) {
					add_action(
						...
						create_function(
							'',
							'echo \'<div id="message" class="error"><p><strong>' .
								sprintf( esc_html__('Please %s to unlock the full program.', 'tvr-microthemer') . '<a target="_blank" href="http://themeover.com/microthemer/">' . esc_html__('purchase Microthemer', 'tvr-microthemer') . '</a>' ) .
							'</strong></p></div>\';'
						)
					);
					return false;
			...
			function getPropertyOptions() {
                ...
                //$this->show_me = __('Font Family', 'tvr-microthemer');
            ...
            function write_mt_version_specific_js() {
                ...
                if (!$write_file = fopen($js_file, 'w')) {
                    $this->log(
                        esc_html__('Permission Error', 'tvr-microthemer'),
                    '<p>' . sprintf( esc_html__('WordPress does not have permission to create: %s', 'tvr-microthemer'), $this->root_rel($js_file) . '. '.$this->permissionshelp ) . '</p>'
                    );
                }
                else {
                    ...
                    <li class="fake-radio-parent <?php echo $hidden; ?>">
                        <label title="<?php echo htmlentities($array['explain']); ?>"><?php echo $array['label']; ?>:</label>
                                ...
                                <input type='radio' autocomplete="off" class='radio'
									...
									<span class="ef-label"><?php esc_html_e('Yes', 'tvr-microthemer'); ?></span>
                                </span>
                                <span class="no-wrap p-wrap-wrap">
                                    <input type='radio' autocomplete="off" class='radio' name='tvr_preferences[<?php echo $key; ?>]' value='0'
                                        ...
                                        />
                                        <span class="fake-radio <?php echo $on; ?>"></span>
										<span class="ef-label"><?php esc_html_e('No', 'tvr-microthemer'); ?></span>
                                </span>
                    </li>
                <?php
                }
			...
			function getRevisions() {
				...
				if ($total_rows == 0) {
					return '<span id="revisions-table">' .
						esc_html__('No Revisions have been created yet. This will happen after your next save.', 'tvr-microthemer') .
					'</span>';
				}
				// if one revision, it's the same as the current settings, explain
				if ($total_rows == 1) {
					return '<span id="revisions-table">' .
						esc_html__('The only revision is a copy of your current settings.', 'tvr-microthemer') .
					'</span>';
				}
            	...
            	<thead>
				<tr>
					<th>' . esc_html__('Revision', 'tvr-microthemer') . '</th>
                    <th>' . esc_html__('Date & Time', 'tvr-microthemer') . '</th>
                    <th>' . esc_html__('User Action', 'tvr-microthemer') . '</th>
					<th>' . esc_html__('Size', 'tvr-microthemer') . '</th>
                    <th>' . esc_html__('Restore', 'tvr-microthemer') . '</th>
                </tr>
				</thead>';
				$i = 0;
				foreach ($revs as $rev) {
					...
					<tr>
						...
						if ($i == 0) {
							...
							$rev_table.='<span class="link restore-link" rel="'.$restore_url.$rev->id.'">' . esc_html__('Restore', 'tvr-microthemer') . '</span>';
						}
						$rev_table.='</td>
					</tr>';
					--$total_rows;
            ...
            function log($short, $long, $type = 'error', $preset = false, $vars = array()){
                ...
                if ($preset) {
                    if ($preset == 'revisions'){
                        ...
                        $this->globalmessage[$this->ei]['long'] = '<p>' . esc_html__('Adding your latest save to the revisions table failed.', 'tvr-microthemer') . '</p>';
                    } elseif ($preset == 'json-decode'){
                        $this->globalmessage[++$this->ei]['short'] = __('Decode json error', 'tvr-microthemer');
                        $this->globalmessage[$this->ei]['type'] = 'error';
                        $this->globalmessage[$this->ei]['long'] = '<p>' . sprintf(esc_html__('WordPress was not able to convert %s into a usable format.', 'tvr-microthemer'), $this->root_rel($vars['json_file']) ) . '</p>';
                    }
            ...
            function display_log(){
                ...
                if (is_array($this->globalmessage)) {
                    $html = '
                    <ul class="logs">'; // so 'loading WP site' msg doesn't overwrite
            ...
            function process_preferences_form(){
                if (isset($_POST['tvr_save_preferences_submit'])) {
                    ...
                    if ($this->savePreferences($pref_array)) {
                        $this->log(
                            esc_html__('Preferences saved', 'tvr-microthemer'), 'tvr-microthemer',
                            '<p>' . esc_html__('Your Microthemer preferences have been successfully updated.', 'tvr-microthemer') . '</p>',
                            'notice'
                        );
			...
			function microthemer_ui_page() {
				...
				if ( isset($_GET['page']) and $_GET['page'] == $this->microthemeruipage ) {
                        ...
                        if (!empty($_GET['action']) and
                            ...
                            if ( !empty($_GET['dir_name'])) {
                                ...
                                if ($library_images = $this->get_linked_library_images($json_config_file)){
                                    foreach($library_images as $key => $root_rel_path){
                                        ...
                                        if (!copy($orig, $new)){
                                            $this->log(
                                                esc_html__('Library image not downloaded', 'tvr-microthemer'), 'tvr-microthemer',
                                                '<p>' . sprintf(esc_html__('%s could not be copied to the zip download file', 'tvr-microthemer'), $root_rel_path) . '</p>',
                                                'warning'
                                            );
                                            $download_status = 0;
                                        }
                            ...
                            if ($library_images){
                                foreach ($img_paths as $key => $path){
                                    if (!unlink($path)){
                                        $this->log(
                                            esc_html__('Temporary image could not be deleted.', 'tvr-microthemer'),
                                            '<p>' . sprintf( esc_html__('%s was temporarily copied to your theme pack before download but could not be deleted after the download operation finished.', 'tvr-microthemer'), $this->root_rel($root_rel_path) ) . '</p>',
                                            'warning'
                                        );
                                    }
                                }
                    ...
                    if( isset($_POST['action']) and $_POST['action'] == 'tvr_microthemer_ui_serialised') {
						$user_action = esc_html__('Save', 'tvr-microthemer');
						...
						if ( $this->saveUiOptions($this->serialised_post['tvr_mcth'])) {
                            $saveOk = esc_html__('Settings Saved', 'tvr-microthemer');
                            $this->log(
                                $saveOk,
                                '<p>' . esc_html__('The UI interface settings were successfully saved.', 'tvr-microthemer') . '</p>',
                                'notice'
                            );
						}
						else {
                            $this->log(
                                esc_html__('Settings failed to save', 'tvr-microthemer'),
                                '<p>' . esc_html__('Saving your setting to the database failed.', 'tvr-microthemer') . '</p>'
                            );
						}
						...
						if ($this->serialised_post['export_to_pack'] == 1) {
							...
							$user_action.= sprintf( esc_html__(' & Export to %s', 'tvr-microthemer'), '<i>'. $this->readable_name($theme). '</i>');
						}
						// else its a standard save of custom settings
						else {
							$theme = 'customised';
							$user_action.= esc_html__(' (regular)', 'tvr-microthemer');
						}
						...
						if ( $max_input_vars ) {
							...
							if ( $num_input_vars >= ($max_input_vars*.5) ) {
                                $this->log(
                                    esc_html__('Data limit near', 'tvr-microthemer'),
                                    '<p>' .
									wp_kses(
					...
					if ( isset($_GET['action']) and $_GET['action'] == 'tvr_microthemer_ui_load_styles') {
                        ...
                        if ($this->debug_pulled_data){
                            ...
                            $write_file = fopen($debug_file, 'w');
                            $data = '';
                            $data.=  __('Custom debug output', 'tvr-microthemer') . "\n\n";
                            $data.= $this->debug_custom;
                            $data.= "\n\n" . __('Last pulled data', 'tvr-microthemer') . "\n\n";
					...
					if ( isset($_POST['import_from_pack_name']) and !empty($_POST['import_from_pack_name']) ) {
                        ...
                        // update the revisions DB field
						$user_action = wp_kses(
							sprintf( esc_html__('Import (%1$s) from %2$s', 'tvr-microthemer'),
								$context,
					...
					elseif( isset($_GET['action']) and $_GET['action'] == 'tvr_ui_reset'){
						...
						if ($this->resetUiOptions()) {
                            ...
                            $this->log(
                                esc_html__('Folders were reset', 'tvr-microthemer'),
                                '<p>' . esc_html__('The default empty folders have been reset.', 'tvr-microthemer') . '</p>',
                                'notice'
                            );
							// update the revisions DB field
							if (!$this->updateRevisions($this->options, 'Settings Reset')) {
                                $this->log(
                                    esc_html__('Revision failed to save', 'tvr-microthemer'),
                                    '<p>' . esc_html__('The revisions table could not be updated.', 'tvr-microthemer') . '</p>',
                                    'notice'
                                );
							}
						}
					...
					elseif(isset($_GET['action']) and $_GET['action'] == 'tvr_clear_styles'){
						...
						if ($this->clearUiOptions()) {
							$this->update_active_styles('customised');
                            $this->log(
                                esc_html__('Styles were cleared', 'tvr-microthemer'),
                                '<p>' . esc_html__("All styles were cleared, but your folders and selectors remain fully intact.", "tvr-microthemer") . '</p>',
                                'notice'
                            );
							// update the revisions DB field
					...
					elseif(isset($_GET['action']) and $_GET['action'] == 'tvr_error_email'){
						...
						if(@mail($to,$subject,$body,$headers)) {
                            $this->log(
                                esc_html__('Email successfully sent', 'tvr-microthemer'),
                                '<p>' . esc_html__('Your error report was successfully emailed to Themeover. Thanks, this really does help.', 'tvr-microthemer') . '</p>',
                                'notice'
                            );
						}
						else {
                            ...
                            $this->log(
                                esc_html__('Report email failed', 'tvr-microthemer'),
                                '<p>' . esc_html__('Your error report email failed to send (are you on localhost?)', 'tvr-microthemer') . '</p>
								<p>' .
								wp_kses(
									sprintf(
					...
					if(isset($_GET['action']) and $_GET['action'] == 'restore_rev'){
						...
						if ($this->restoreRevision($rev_key)) {
                            $this->log(
                                esc_html__('Settings successfully restored', 'tvr-microthemer'),
                                '<p>' . esc_html__('Your settings were successfully restored from a previous save.', 'tvr-microthemer') . '</p>',
                                'notice'
                            );
							$this->update_active_styles('customised');
							// update the revisions DB field
                            ...
                            $this->log(
                                esc_html__('Settings restore failed', 'tvr-microthemer'),
                                '<p>' . esc_html__('Data could not be restored from a previous save.', 'tvr-microthemer') . '</p>'
                            );
						}
                        // save last message in database so that it can be displayed on page reload (just once)
                        $this->cache_global_msg();
                    ...
                    if (isset($_POST['tvr_update_media_queries_submit'])){
                        ...
                        if ($this->savePreferences($pref_array)) {
                            switch ($action) {
                                case 'reset':
                                    $this->log(
                                        esc_html__('Media queries were reset', 'tvr-microthemer'),
                                        '<p>' . esc_html__('The default media queries were successfully reset.', 'tvr-microthemer') . '</p>',
                                        'notice'
                                    );
                                    break;
                                case 'update':
                                    $this->log(
                                        esc_html__('Media queries were updated', 'tvr-microthemer'),
                                        '<p>' . esc_html__('Your media queries were successfully updated.', 'tvr-microthemer') . '</p>',
                                        'notice'
                                    );
                                    break;
                                case 'load_set':
                                    $this->log(
                                        esc_html__('Media query set loaded', 'tvr-microthemer'),
                                        '<p>' . sprintf( esc_html__('A new media query set %1$s your existing media queries: %2$s', 'tvr-microthemer'), $load_action, htmlentities($_POST['tvr_preferences']['load_mq_set']) ) . '</p>',
                                        'notice'
                                    );
                                    break;
                            }
                    ...
                    if (isset($_POST['tvr_ui_validate_submit'])) {
                        ...
                        if ($validation) {
                            ...
                            if (!$this->preferences['buyer_validated']) { // not already validated
                                $this->log(
                                    esc_html__('Full program unlocked!', 'tvr-microthemer'),
                                    '<p>' . esc_html__('Your email address has been successfully validated. Microthemer\'s full program features have been unlocked!', 'tvr-microthemer') . '</p>',
                                    'notice'
                                );
                            } else {
                                $this->log(
                                    esc_html__('Already validated', 'tvr-microthemer'),
                                    '<p>' . esc_html__('Your email address has already been validated. The full program is currently active.', 'tvr-microthemer') . '</p>',
                                    'notice'
                                );
                            }
                        }
                            ...
                            $this->trial = true;
                            $this->log(
                                esc_html__('Validation failed', 'tvr-microthemer'),
                                '<p>' . esc_html__('Your email address could not be validated. Please try the following:', 'tvr-microthemer') . '</p>
								<ul>' .
									'<li>' . wp_kses(
										sprintf(
									...
									'<li>' . wp_kses(
										sprintf(
											__('If you purchased Microthemer from <b>CodeCanyon</b>, please send us a "Validate my email" message via the contact form on the right hand side of <a %s>this page</a> (you will need to ensure that you are logged in to CodeCanyon).', 'tvr-microthemer'),
											'target="_blank" href="http://codecanyon.net/user/themeover"' ),
										array( 'a' => array( 'href' => array(), 'target' => array() ), 'b' => array() )
									) . '</li>' .
									'<li>' . wp_kses(
										sprintf(
											__('The connection to Themeover\'s server may have failed due to an intermittent network error. <span %s>Resubmitting your email one more time</span> may do the trick.', 'tvr-microthemer'),
									...
									'<li>' . esc_html__('Security settings on your server may block all outgoing PHP connections to domains not on a trusted whitelist (e.g. sites that are not wordpress.org). Ask your web host about unblocking themeover.com - even if they just do it temporarily to allow you to activate the plugin.', 'tvr-microthemer') . '</li>'
                                . '</ul>
								<p>' .
								wp_kses(
									sprintf(
										__('If none of the above solve your problem please send WP login details for the site you\'re working on using <a %1$s>this secure contact form</a>. Please remember to include the URL to your website along with the username and password. If you\'d prefer not to provide login details please send us an email via the <a %1$s>contact form</a> to discuss alternative measures.', 'tvr-microthemer'),
                        ...
                        if (!$this->savePreferences($pref_array)) {
                            $this->log(
                                esc_html__('Unlock status not saved', 'tvr-microthemer'),
                                '<p>' . esc_html__('Your validation status could not be saved. The program may need to be unlocked again.', 'tvr-microthemer') . '</p>'
                            );
                        }
                    }
                    ...
                    if (isset($_POST['tvr_preferences_reset'])) {
                        ...
                        if ($this->savePreferences($pref_array)) {
                            $this->log(
                                esc_html__('Preferences were reset', 'tvr-microthemer'),
                                '<p>' . esc_html__('The default program preferences were reset.', 'tvr-microthemer') . '</p>',
                                'notice'
                            );
                        }
                    }
			...
			function manage_micro_themes_page() {
				...
				if ( $_GET['page'] == $this->microthemespage ) {
                    ...
                    if (!empty($_GET['action']) and $_GET['action'] == 'tvr_delete_ok') {
                        ...
                        $this->log(
                            esc_html__('Design pack deleted', 'tvr-microthemer'),
                            '<p>' . esc_html__('The design pack was successfully deleted.', 'tvr-microthemer') . '</p>',
                            'notice'
                        );
                    }
					...
					if (isset($_POST['tvr_create_micro_submit'])) {
						...
						if ( !empty($micro_name) ) {
                            ...
                            $this->log(
                                esc_html__('Please specify a name', 'tvr-microthemer'),
                                '<p>' . esc_html__('You didn\'t enter anything in the "Name" field. Please try again.', 'tvr-microthemer') . '</p>'
                            );
					...
					if (
						...
						// update the revisions DB field
						$user_action = sprintf(
							esc_html__('%s Activated', 'tvr-microthemer'), 
							'<i>' . $this->readable_name($theme_name) . '</i>'
						);
						if (!$this->updateRevisions($this->options, $user_action)) {
                            $this->log('', '', 'error', 'revisions');
					...
					if (
						...
						if ($this->savePreferences($pref_array)) {
                            $this->log(
                                esc_html__('Item deactivated', 'tvr-microthemer'),
								'<p>' .
								sprintf(
									esc_html__('%s was deactivated.', 'tvr-microthemer'),
            ...
            function manage_single_page() {
                ...
                if( $_GET['page'] == $this->managesinglepage ) {
                    ...
                    if (
                        ...
                        if ($delete_ok){
                            $this->log(
                                esc_html__('File deleted', 'tvr-microthemer'),
                                '<p>' . sprintf( esc_html__('%s was successfully deleted.', 'tvr-microthemer'), htmlentities($root_rel_path) ) . '</p>',
                                'notice'
                            );
                            ...
                            $this->log(
                                esc_html__('File delete failed', 'tvr-microthemer'),
                                '<p>' . sprintf( esc_html__('%s was not deleted.', 'tvr-microthemer'), htmlentities($root_rel_path) ) . '</p>'
                            );
                        }
                    }
			...
			***/
			...
			function plugin_menu() {
				...
				<a id="tvr-item-1" href='admin.php?page=<?php echo $this->microthemeruipage;?>' title="<?php esc_attr_e('Go to Microthemer UI Page', 'tvr-microthemer') ?>">UI</a>
				<a id="tvr-item-2" href='admin.php?page=<?php echo $this->microthemespage;?>' title="<?php esc_attr_e('Go to Manage Micro Themes Page', 'tvr-microthemer') ?>">Manage</a>
				<a id="tvr-item-3" href='admin.php?page=<?php echo $this->preferencespage;?>' title="<?php esc_attr_e('Go to Microthemer Preferences Page', 'tvr-microthemer') ?>">Options</a>
            	</div>
                </div>
                <?php
			}
			...
			function need_help_notice() {
				if ($this->preferences['need_help'] == '1' and TVR_MICRO_VARIANT != 'loader') {
					?>
					<p class='need-help'><b><?php esc_html_e('Need Help?', 'tvr-microthemer'); ?></b>
					<?php printf(
						wp_kses(
							__('Browse Our <span %1$s>Video Guides</span> and <span %2$s>Tutorials</span> or <span %3$s>Search Our Forum</span>', 'tvr-microthemer'),
							array( 'span' => array() )
						),
			...
			function ie_notice() {
				...
				if ($this->preferences['ie_notice'] == '1' and $this->check_browser() != 'Chrome') {
                    $this->log(
                        esc_html__('Chrome Is Faster', 'tvr-microthemer'),
						'<p>' .
						sprintf(
			...
			function validate_reminder() {
				if (!$this->preferences['buyer_validated'] and TVR_MICRO_VARIANT == 'themer') {
					...
					<p><b><?php esc_html_e("IMPORTANT - Free Trial Mode is Active", "tvr-microthemer"); ?></b><br /> <br />
						<?php wp_kses(
						...
						<br />
						<?php esc_html__('The Free Trial limits you to editing or creating 3 Sections and 9 Selectors (3 per Section).', 'tvr-microthemer'); ?>
					</p>
					<p><?php wp_kses(
            ...
            function combined_devices(){
                $comb_devs['all-devices'] = array(
                    'label' => esc_html__('All Devices', 'tvr-microthemer'),
                    'query' => esc_html__('General CSS that will apply to all devices', 'tvr-microthemer'),
                    'min' => 0,
                    'max' => 0
                );
			...
			function dir_loop($dir_name) {
				...
				if ( !is_dir($dir_name) ) {
					if ( !wp_mkdir_p($dir_name) ) {
                        $this->log(
                            esc_html__('/micro-themes folder error', 'tvr-microthemer'),
							'<p>' .
							sprintf(
								esc_html__('WordPress was not able to create the %1$s directory. ', 'tvr-microthemer'),
			...
			***/
                    ...
                    <li class="upload">
                        <form name='upload_micro_form' id="upload-micro-form" method="post" enctype="multipart/form-data"
                            ...
                            <input class="tvr-button upload-pack" type="submit" name="tvr_upload_micro_submit"
								value="<?php esc_attr_e('+ Upload New', 'tvr-microthemer'); ?>" title="<?php esc_attr_e('Upload a new design pack', 'tvr-microthemer'); ?>" />
                        </form>
                    </li>
                    <!--<li>
            ...
            function pack_pagination($page, $total_pages, $total_packs, $start, $end) {
                ...
                <ul class="tvr-pagination">
					...
					echo '<li class="displaying-x">' .
						sprintf(esc_html__('Displaying %1$s - %2$s of %3$s', 'tvr-microthemer'), $start, $end, $total_packs) . '</li>';
                    ...
                    if (!empty($this->preferences['theme_in_focus']) and $total_packs > 0){
                        ...
                        <li class="last-modified" rel="<?php echo $this->preferences['theme_in_focus']; ?>">
							<?php esc_html_e('Last modified design pack: ', 'tvr-microthemer'); ?><a title="<?php printf(esc_attr__('Edit %s', 'tvr-microthemer'), $name); ?>"
                               href="<?php echo $url; ?>"><?php esc_html($name); ?>
                            </a>
                        </li>
                        <?php
                    }
            ...
            function display_left_menu_icons() {
                ...
                if ($this->preferences['buyer_validated']){
                    ...
                    $unlock_title = esc_attr__('Validate license using a different email address', 'tvr-microthemer');
                } else {
                    $unlock_class = '';
                    $unlock_title = esc_attr__('Enter your PayPal email (or the email listed in My Downloads) to unlock Microthemer', 'tvr-microthemer');
                }
                ...
                $html = '
                    ...
                    <div class="save-interface v-left-button" title="' . esc_attr__("Manually save UI settings (Ctrl+S)", "tvr-microthemer") . '"></div>

                    <div id="toggle-highlighting" class="v-left-button"
                    title="'. esc_attr__("Toggle highlighting", "tvr-microthemer") .'"></div>

                    <div id="toggle-rulers" class="toggle-rulers v-left-button '.$ruler_class.'"
                        title="'. esc_attr__('Toggle rulers on/off', 'tvr-microthemer') .'"></div>
                    ...
                    <div class="ruler-tools v-left-button tvr-popright-wrap">
                        ...
                        title="'. esc_attr__('Remove all guides', 'tvr-microthemer') .'"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="code-tools v-left-button tvr-popright-wrap popup-show">
                        ...
                        title="'. esc_attr__('Code editor view', 'tvr-microthemer') .'"></div>

                        <div class="tvr-popright">
                            <div class="popright-sub">
                                <div class="display-css-code v-left-button show-dialog popup-show" rel="display-css-code" title="' . esc_attr__("View the CSS code Microthemer generates", "tvr-microthemer") . '"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="preferences-tools v-left-button tvr-popright-wrap popup-show">
                        ...
                        <div class="display-preferences v-left-button show-dialog popup-show" rel="display-preferences" title="' . esc_attr__("Set your global Microthemer preferences", "tvr-microthemer") . '"></div>

                        <div class="tvr-popright">
                            <div class="popright-sub">

                                <div class="edit-media-queries v-left-button show-dialog popup-show" rel="edit-media-queries"
                    title="' . esc_attr__("Edit Media Queries", "tvr-microthemer") . '"></div>
                    ...
                    <div class="pack-tools v-left-button tvr-popright-wrap popup-show">
                        ...
                        <div class="manage-design-packs v-left-button show-dialog new-icon-group popup-show" rel="manage-design-packs" title="' . esc_attr__("Install & Manage your design packages", "tvr-microthemer") . '"></div>

                        <div class="tvr-popright">
                            <div class="popright-sub">

                                <div class="import-from-pack v-left-button show-dialog popup-show" rel="import-from-pack" title="' . esc_attr__("Import settings from a design pack", "tvr-microthemer") . '"></div>

                    <div class="export-to-pack v-left-button show-dialog popup-show" rel="export-to-pack" title="' . esc_attr__("Export your work as a design pack", "tvr-microthemer") . '"></div>
                    ...
                    <!--<div class="display-share v-left-button show-dialog" rel="display-share" title="' . esc_attr__("Spread the word about Microthemer", "tvr-microthemer") . '"></div>-->
                    ...
                    <div class="reset-tools v-left-button tvr-popright-wrap popup-show">
                        ...
                        <div class="display-revisions v-left-button show-dialog new-icon-group popup-show" rel="display-revisions" title="' . esc_attr__("Restore settings from a previous save point", "tvr-microthemer") . '"></div>

                        <div class="tvr-popright">
                            <div class="popright-sub">
                                <div id="ui-reset" class="v-left-button folder-reset"
                                title="' . esc_attr__("Reset the interface to the default empty folders", "tvr-microthemer") . '"></div>
                                <div id="clear-styles" class="v-left-button styles-reset"
                                title="' . esc_attr__("Clear all styles, but leave folders and selectors intact", "tvr-microthemer") . '"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    title="' . esc_attr__("Learn how to use Microthemer", "tvr-microthemer") . '"></div>

                    <div class="toggle-full-screen v-left-button" rel="toggle-full-screen"
                    title="' . esc_attr__("Full screen mode", "tvr-microthemer") . '"></div>

                    <a class="back-to-wordpress v-left-button" title="' . esc_attr__("Return to WordPress dashboard", "tvr-microthemer") . '"
                    ...
                    title="View Microthemer\'s integration options">
                        <div class="tvr-popright integration-right">
                            <div class="popright-sub integration-sub">
                                ...
                                title="' . esc_attr__("Enable/Disable WPTouch mode", "tvr-microthemer") . '">WPTouch</div>
                                <div id="toggle-beaver" class="v-left-button toggle-beaver"
                                title="' . esc_attr__("Enable/Disable Builder Beaver mode", "tvr-microthemer") . '">Builder Beaver</div>
                            </div>
                        </div>
                    </div>
                 */
            ...
            function menu_section_html($section_name, $array) {
                ...
                <li id="<?php echo 'strk-'.$section_name; ?>" class="section-tag strk strk-sec">
					...
					<span class="folder-icon tvr-icon sortable-icon" title="<?php esc_attr_e("Reorder folder", "tvr-microthemer"); ?>"></span>
					<span class="section-name item-name"
                          <?php /*title="<?php esc_attr_e("Folder", "tvr-microthemer"); ?>" */ ?>
                        >
                        ...
                        <span class="edit-section-form hidden">
                            <input type='text' class='rename-input' name='rename_section'
								   ...
								   <span class='rename-button tvr-button' title="<?php esc_attr_e("Rename folder", "tvr-microthemer"); ?>"><?php printf( esc_html__('Rename', 'tvr-microthemer') ); ?></span>
								   <span class='cancel-rename-section cancel link' title="<?php esc_attr_e("Cancel rename", "tvr-microthemer"); ?>"><?php printf( esc_html__('Cancel', 'tvr-microthemer') ); ?></span>
                        </span>
                        <span class="manage-section-icons manage-icons">

						<span class="delete-section tvr-icon delete-icon" title="<?php esc_attr_e("Delete Folder", "tvr-microthemer"); ?>"></span>
							<span class="copy-section tvr-icon copy-icon" title="<?php esc_attr_e("Copy Folder", "tvr-microthemer"); ?>"></span>
							<span class="toggle-rename-section tvr-icon edit-icon" title="<?php esc_attr_e("Rename Folder", "tvr-microthemer"); ?>"></span>
							<span class="reveal-add-selector tvr-icon add-icon" title="<?php esc_attr_e("Add selector to this folder", "tvr-microthemer"); ?>"></span>
                        </span>
                    </div>
                        ...
                        <ul class="selector-sub">
                            <li class="add-selector-list-item">
                                <div class="sel-row item-row">
                                    <?php
                                    $tip = esc_html__('Non-coders should use the selector wizard instead of using these form fields. Just double-click something on your site!', 'tvr-microthemer');
                                    if (!$this->optimisation_test){
            ...
            function menu_selector_html($section_name, $css_selector, $array, $sel_loop_count) {
                ...
                <li id="<?php echo 'strk-'.$section_name.'-'.$css_selector; ?>" class="selector-tag strk strk-sel <?php echo $sel_class; ?>">
                    ...
                    <div class="sel-row item-row">
					    <span class="tvr-icon selector-icon sortable-icon" title="<?php esc_attr_e("Reorder or move selector into a different folder", "tvr-microthemer"); ?>"></span>
						...
						<span class="delete-selector tvr-icon delete-icon" title="<?php esc_attr_e("Delete Selector", "tvr-microthemer"); ?>"></span>
							<span class="copy-selector tvr-icon copy-icon" title="<?php esc_attr_e("Copy Selector", "tvr-microthemer"); ?>"></span>
							<span class="toggle-modify-selector tvr-icon edit-icon" title="<?php esc_attr_e("Edit Selector", "tvr-microthemer"); ?>"></span>
                        </span>
                        <?php
                        $tip = esc_html__('Give your selector a better descriptive name and/or modify the CSS selector code (if you know CSS)', 'tvr-microthemer');
                        if (!$this->optimisation_test){
            ...
            function selector_add_modify_form($con, $tip, $labelCss = '', $section_name ='', $css_selector = '') {
                ...
                <div class='<?php echo $con; ?>-selector-form float-form hidden'>
                    ...
                    <p>
						<label><?php esc_html_e('Descriptive Name:', 'tvr-microthemer'); ?></label>
                        <input type='text' class='selector-name-input' name='<?php echo $con; ?>_selector[label]' value='<?php esc_attr($display_selector_name); ?>' />
                    </p>
                    <p>
                       	<label><?php esc_html_e('CSS Selector Code:', 'tvr-microthemer'); ?></label>
                    ...
                    name="tvr_mcth['.$section_name.']['.$css_selector.']['.$poly.']" value="1" />';
                                    ...
                                    <span class="<?php echo $poly; ?>-toggle tvr-icon input-icon-toggle <?php echo $class; ?>"
                                          ...
                                          title='<?php printf(esc_attr__('Click to turn the %1$s polyfill on or off for this selector. %2$s', 'tvr-microthemer'), $poly, $extra_explain); ?>'
                        ...
                        <span class='<?php echo $con; ?>-selector tvr-button'
								title="<?php printf(esc_attr__("%s Selector", "tvr-microthemer"), $display); ?>"><?php printf(esc_html__('%s Selector', 'tvr-microthemer'), $display ); ?></span>
							  <span class="cancel-<?php echo $con; ?>-selector cancel link" title="<?php printf(esc_attr__('Cancel %s Selector', 'tvr-microthemer'), $display); ?>"><?php esc_html_e('Cancel', 'tvr-microthemer'); ?></span>
                    </p>
                </div>
            <?php
            }
            ...
            function edit_mq_row(
                ...
                $template = true){
                    ...
                    <li class="mq-row <?php echo $li_class; ?>">
                        <span class="del-m-query tvr-icon delete-icon" title="<?php esc_attr_e("Delete this media query", "tvr-microthemer"); ?>"></span>
                        <div class="mq-edit-wrap mq-label-wrap">
                            <label title="<?php esc_attr_e('Give your media query a descriptive name', 'tvr-microthemer'); ?>"><?php esc_html_e('Label:', 'tvr-microthemer'); ?></label>
                        ...
                        <div class="mq-edit-wrap mq-query-wrap">
                            <label title="<?php esc_attr_e("Set the media query condition", "tvr-microthemer"); ?>">
                                <?php esc_html_e('Media Query:', 'tvr-microthemer'); ?></label>
                        ...
                        <div class="mq-edit-wrap mq-checkbox-wrap">
                            <label title="<?php esc_attr_e("Hide this tab in the interface if you don't need it right now and you'd like to keep the number of tabs in the interface manageably low", "tvr-microthemer"); ?>">
								<?php esc_html_e('Hide Tab In Interface', 'tvr-microthemer'); ?>:</label>
                            <?php
                            $checked = '';
                            $on = '';
                            ...
                            <span class="ef-label">
                                <?php esc_attr_e('Yes (no settings will be lost)', 'tvr-microthemer'); ?>
                            </span>
                        </div>
                    </li>
                <?php
            ...
            function global_media_query_tabs(){
                ...
                <div class="query-tabs">
                    ...
                    title="' . esc_attr__('Edit media queries', 'tvr-microthemer') . '" rel="edit-media-queries">
                    </span>
			...
			function media_query_tabs($section_name, $css_selector, $property_group_name,
				...
				rel="all-devices" title="' . esc_attr__('General CSS that will apply to all devices', 'tvr-microthemer') . '">' . esc_html__('All Devices', 'tvr-microthemer') . '</span>';
				foreach ($this->preferences['m_queries'] as $key => $m_query) {
			...
			function media_query_checkboxes($section_name, $css_selector,
                ...
                <span class="mq-button mqb-all-devices '.$property_group_state.'" title="' . __('Styles will apply to all screen sizes', 'tvr-microthemer') . '">';
					...
					<span class="mq-button-text mq-add" rel="all-devices|All Devices" title="' . __('Styles will apply to all screen sizes', 'tvr-microthemer') . '">' .
						esc_html__('All Devices', 'tvr-microthemer') . '</span>
                ...
                <span class="mq-button remove-all" title="' . sprintf(__('Remove all "%1$s" styling options for this selector', 'tvr-microthemer'), ucwords($property_group_name)) . '">
					...
					<span class="mq-button-text mq-remove-all link">' . esc_html__('Remove all tabs', 'tvr-microthemer') . '</span>
                </span>';
					...
					if ( is_array($array) ) {
						...
						foreach ( $array as $css_selector => $array) {
							...
							if (!empty($array['styles']) and !is_array($array['styles'])) {
                                ...
                                $this->log(
                                    esc_html__('Malformed error', 'tvr-microthemer'),
                                    '<p>'.$section_name. ' &raquo; '.$css_sel_name.'</p>
									<p>' .
									sprintf(
										esc_html__('To fix this issue, please delete the malformed \'%1$s\' selector (which may not be displaying a name) in the \'%2$s\' folder', 'tvr-microthemer'),
										$css_sel_name, $section_name
									) . '</p>' .
									esc_html__('Array is: ', 'tvr-microthemer') . $array['styles']
                                );
...
';
					...
					if (!fwrite($write_file, $sty['data'])) {
                        $this->log(
                            esc_html__('Write stylesheet error', 'tvr-microthemer'),
                            '<p>' . sprintf(esc_html__('Writing to %s failed. ', 'tvr-microthemer'), $this->root_rel($act_styles)) . $this->permissionshelp.'</p>'
                        );
					}
					fclose($write_file);
					}
					// no write access
                        ...
                        $this->log(
                            esc_html__('Write stylesheet error', 'tvr-microthemer'),
							'<p>' . esc_html__('WordPress does not have "write" permission	for: ', 'tvr-microthemer')
					...
					if ($this->savePreferences($pref_array) and $activated_from != 'customised') {
                        $this->log(
                            esc_html__('Design pack activated', 'tvr-microthemer'),
                            '<p>' . esc_html__('The design pack was successfully activated.', 'tvr-microthemer') . '</p>',
                            'dev-notice'
                        );
					}
						...
						if (!empty($trim_val)) {
							...
							if (!file_exists($stylesheet)) {
								if (!$write_file = fopen($stylesheet, 'w')) {
                                    $this->log(
                                        esc_html__('Create IE stylesheet error', 'tvr-microthemer'),
										'<p>' . esc_html__('WordPress does not have permission to create: ', 'tvr-microthemer')
							...
							if ( is_writable($stylesheet) )  {
								...
								if (!fwrite($write_file, stripslashes($val))) {
                                    $this->log(
                                        esc_html__('Write IE stylesheet error', 'tvr-microthemer'),
                                        '<p>' . sprintf(esc_html__('Writing to %s failed. ', 'tvr-microthemer'), $this->root_rel($stylesheet))
                                        . $this->permissionshelp . '</p>'
                                    );
								}
								fclose($write_file);
			...
			function update_json_file($theme, $context = '', $secondary_content = '') {
				...
				if (!file_exists($json_file)) {
					if (!$write_file = fopen($json_file, 'w')) { // this creates a blank file for writing
                        $this->log(
                            esc_html__('Create json error', 'tvr-microthemer'),
							'<p>' . esc_html__('WordPress does not have permission to create: ', 'tvr-microthemer')
				...
				if ($context != 'create-blank') {
					if ( is_writable($json_file) )  {
                        ...
                        if ( empty($this->serialised_post['export_sections']['hand_coded_css'])) {
                            ...
                            echo esc_html(_x('unset: hand-coded', 'Verb', 'tvr-microthemer')) . '<br />';
                        } else {
                            echo wp_kses(__('<b>don\'t</b> unset: hand-coded', 'tvr-microthemer'), array( 'b' => array())) . '<br />';
                        }
                        // ie too
                        foreach ($this->preferences['ie_css'] as $key => $value) {
                            if ( empty($this->serialised_post['export_sections']['ie_css'][$key])) {
                                ...
                                echo esc_html(_x('unset: ', 'Verb', 'tvr-microthemer')) . $key. '<br />';
                            } else {
                        ...
                        if ($this->debug_selective_export) {
                            ...
                            $write_file = fopen($debug_file, 'w');
                            $data.= esc_html__('The Selectively Exported Options', 'tvr-microthemer') . "\n\n";
                            $data.= print_r($json_data, true);
                            $data.= "\n\n" . esc_html__('The Full Options', 'tvr-microthemer') . "\n\n";
                            $data.= print_r($this->options, true);
                            fwrite($write_file, $data);
                            fclose($write_file);
                        }
						...
						if ($data = $json_object->encode($json_data)) {
                            ...
                            $this->log(
                                esc_html__('Encode json error', 'tvr-microthemer'),
                                '<p>' . esc_html__('WordPress failed to convert your settings into json.', 'tvr-microthemer') . '</p>'
                            );
						}
					}
					else {
                        $this->log(
                            esc_html__('Write json error', 'tvr-microthemer'),
							'<p>' . esc_html__('WordPress does not have "write" permission	for: ', 'tvr-microthemer')
							. $this->root_rel($json_file) . '. '.$this->permissionshelp.'</p>'
                        );
					}
            ...
            function load_json_file($json_file, $theme_name, $context = '') {
                ...
                if (file_exists($json_file)) {
                    ...
                    if (!$read_file = fopen($json_file, 'r')) {
                        $this->log(
                            esc_html__('Read json error', 'tvr-microthemer'),
							'<p>' . sprintf( esc_html__('WordPress was not able to read %s. ', 'tvr-microthemer'), $this->root_rel($json_file) )
							. '. '.$this->permissionshelp.'</p>'
                        );
                    }
                    else {
                    ...
                    if (!$json_error) {
                        ...
                        // import success
                        $this->log(
                            esc_html__('Settings were imported', 'tvr-microthemer'),
							'<p>' . esc_html__('The design pack settings were successfully imported.', 'tvr-microthemer') . '</p>',
                            'notice'
                        );
                        ...
                        if ($mq_analysis['new']) {
                            ...
                            $this->log(
                                esc_html__('Media queries added', 'tvr-microthemer'),
                ...
                // the config file doesn't exist
                    ...
                    $this->log(
                        esc_html__('Config file missing', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('%1$s settings file could not be loaded because it doesn\'t exist in %2$s', 'tvr-microthemer'),
            ...
            function merge($orig_settings, $new_settings) {
                ...
                if (is_array($new_settings)) {
                    ...
                    if ($this->debug_merge) {
                        $data .= "\n\n" . esc_html__('The to merge options', 'tvr-microthemer') . "\n\n";
			...
			***/
            ...
            function setup_micro_themes_dir(){
                if ( !is_dir($this->micro_root_dir) ) {
                    if ( !wp_mkdir_p( $this->micro_root_dir, 0755 ) ) {
                        $this->log(
                            esc_html__('/micro-themes create error', 'tvr-microthemer'),
							'<p>' . sprintf(
								esc_html__('WordPress was not able to create the %s directory. ', 'tvr-microthemer'),
            ...
            function maybe_create_stylesheet($act_styles){
                ...
                if (!file_exists($act_styles)) {
                    if (!$write_file = fopen($act_styles, 'w')) {
                        $this->log(
                            esc_html__('Create stylesheet error', 'tvr-microthemer'),
                            '<p>' . esc_html__('WordPress does not have permission to create: ', 'tvr-microthemer') .
                            $this->root_rel($act_styles) . '. '.$this->permissionshelp.'</p>'
                        );
                        $error = true;
            ...
            function copy_pie(){
                ...
                foreach($pie_files as $file){
                    ...
                    if (!copy($orig, $new)){
                        $this->log(
                            esc_html__('CSS3 PIE not copied', 'tvr-microthemer'),
							'<p>' . sprintf(
								esc_html__('CSS3 PIE (%s) could not be copied to correct location. This is needed to support gradients, rounded corners and box-shadow in old versions of Internet Explorer.', 'tvr-microthemer'),
								$file
							) . '</p>',
                            'error'
                        );
			...
			function create_micro_theme($micro_name, $action, $temp_zipfile) {
				...
				if ( !is_writeable( $this->micro_root_dir ) ) {
                    $this->log(
                        esc_html__('/micro-themes write error', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('The directory %s is not writable. ', 'tvr-microthemer'),
							$this->root_rel($this->micro_root_dir)
						) . $this->permissionshelp . '</p>'
                    );
					$error = true;
				...
				if ( !wp_mkdir_p ( $this_micro_abs ) ) {
                    $this->log(
                        esc_html__('design pack create error', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('WordPress was not able to create the %s directory.', 'tvr-microthemer'), $this->root_rel($this_micro_abs)
						). '</p>'
                    );
					$error = true;
				}
				...
				if ( !is_writeable( $this_micro_abs ) ) {
                    $this->log(
						esc_html__('design pack write error', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('The directory %s is not writable. ', 'tvr-microthemer'), $this->root_rel($this_micro_abs)
						) . $this->permissionshelp . '</p>'
                    );
					$error = true;
				}
				if (SAFE_MODE and $this->preferences['safe_mode_notice'] == '1') {
                    $this->log(
                        esc_html__('Safe-mode is on', 'tvr-microthemer'),
						'<p>' . esc_html__('The PHP server setting "Safe-Mode" is on.', 'tvr-microthemer')
						. '</p><p>' . sprintf(
				...
				if ($error != true) {
					if ($action == 'create') {
                        $this->log(
                            esc_html__('Design pack created', 'tvr-microthemer'),
                            '<p>' . esc_html__('The design pack directory was successfully created on the server.', 'tvr-microthemer') . '</p>',
                            'notice'
                        );
					}
					if ($action == 'unzip') {
                        $this->log(
                            esc_html__('Design pack installed', 'tvr-microthemer'),
							...
							' <span class="show-parent-dialog link" rel="import-from-pack">' . esc_html__('import option', 'tvr-microthemer') . '</span>'.
					...
					if ($action == 'export') {
                        $this->log(
                            esc_html__('Settings exported', 'tvr-microthemer'),
                            '<p>' . esc_html__('Your settings were successfully exported as a design pack directory on the server.', 'tvr-microthemer') . '</p>',
                            'notice'
                        );
					}
				}
            ...
            function rename_from_meta($meta_file, $name){
                ...
                if (is_file($meta_file) and is_readable($meta_file)) {
                    ...
                    // no meta file error
                    $this->log(
                        esc_html__('Missing meta file', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('The zip file doesn\'t contain a necessary %s file or it could not be read.', 'tvr-microthemer'),
            ...
            function get_file_data($file){
                if (!is_file($file)){
                    $this->log(
                        esc_html__('File doesn\'t exist', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('%s does not exist on the server.', 'tvr-microthemer'),
							$this->root_rel($file)
						) . '</p>'
                    );
                    return false;
                ...
                if (!is_readable($file)){
                    $this->log(
                        esc_html__('File not readable', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__(' %s could not be read.', 'tvr-microthemer'),
							$this->root_rel($file)
						) . '</p>'
                        . $this->permissionshelp
                    );
            ...
            function import_pack_images_to_library($json_config_file, $name){
                ...
                foreach ($img_array[2] as $key => $config_img_path) {
                    ...
                    if (file_exists($pack_image)) {
                        ...
                        if ( $id === 0 or is_wp_error($id) ) {
                            ...
                            $this->log(
                                esc_html__('Move to media library failed', 'tvr-microthemer'),
								'<p>' . sprintf(
									esc_html__('%s was not imported due to an error.', 'tvr-microthemer'),
                ...
                if ($moved){
                    $this->log(
                        esc_html__('Images transferred to media library', 'tvr-microthemer'),
                        '<p>' . esc_html__('The following images were transferred from the design pack to your WordPress media library:', 'tvr-microthemer') . '</p>'
                        . $moved_list,
                        'notice'
                    );
                    // update paths in json file
                ...
                if (is_writable($json_config_file)) {
                    if ($write_file = fopen($json_config_file, 'w')) {
                        if (fwrite($write_file, $data)) {
                            fclose($write_file);
                            $this->log(
                                esc_html__('Images paths updated', 'tvr-microthemer'),
                                '<p>' . esc_html__('Images paths were successfully updated to reflect the new location or deletion of an image(s).', 'tvr-microthemer') . '</p>',
                                'notice'
                            );
                        }
                        else {
                            $this->log(
                                esc_html__('Image paths failed to update.', 'tvr-microthemer'),
								'<p>' . esc_html__('Images paths could not be updated to reflect the new location of the images transferred to your media library. This happened because Microthemer could not rewrite the config.json file.', 'tvr-microthemer') . '</p>' . $this->permissionshelp
                            );
                        }
                    }
                }
            ...
            function import_image_to_library($file, $just_image_name, $post_id = 0, $import_date = false) {
                ...
                if ( ! ( ( $uploads = wp_upload_dir($time) ) && false === $uploads['error'] ) ) {
                    $this->log(
                        esc_html__('Uploads folder error', 'tvr-microthemer'),
                        $uploads['error']
                    );
                    return 0;
                }
                ...
                if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) ) {
                    $this->log(
                        esc_html__('Wrong file type', 'tvr-microthemer'),
                        '<p>' . esc_html__('Sorry, this file type is not permitted for security reasons.', 'tvr-microthemer') . '</p>'
                    );
                    return 0;
                }
                ...
                if ( preg_match('|^' . preg_quote(str_replace('\\', '/', $uploads['basedir'])) . '(.*)$|i', $file, $mat) ) {
                    ...
                    if ( !empty($attachment) ) {
                        $this->log(
                            esc_html__('Image already in library', 'tvr-microthemer'),
							'<p>' . sprintf(
								esc_html__('%s already exists in the WordPress media library and was therefore not moved', 'tvr-microthemer'),
								$filename
							) . '</p>',
                            'warning'
                        );
                    ...
                    if ( false === @rename( $file, $new_file ) ) {
                        $this->log(
                            esc_html__('Move to library failed', 'tvr-microthemer'),
							'<p>' . sprintf(
								esc_html__('%1$s could not be moved to %2$s', 'tvr-microthemer'),
								$filename,
								$uploads['path']
							) . '</p>',
                            'warning'
            ...
            function handle_zip_package() {
                ...
                if ( !preg_match('/chrome/i', $_SERVER['HTTP_USER_AGENT']) ) {
                    ...
                    if ( !preg_match('/(zip|download|octet-stream)/i', $_FILES['upload_micro']['type']) ) {
                        ...
                        $this->log(
                            esc_html__('Faulty zip file', 'tvr-microthemer'),
                            '<p>' . esc_html__('The uploaded file was faulty or was not a zip file.', 'tvr-microthemer') . '</p>
						<p>' . esc_html__('The server recognised this file type: ', 'tvr-microthemer') . $_FILES['upload_micro']['type'].'</p>'
                        );
                    }
                }
			...
			function extract_files($dir, $file) {
				...
				if ($archive->extract(PCLZIP_OPT_PATH, $dir, PCLZIP_OPT_REMOVE_ALL_PATH,
                    ...
                    $this->log(
                        esc_html__('Extract zip error', 'tvr-microthemer'),
                        '<p>' . esc_html__('Error : ', 'tvr-microthemer') . $archive->errorInfo(true).'</p>'
                    );
				}
			}
			...
			function create_zip($path_to_dir, $dir_name, $zip_store) {
				...
				if (is_writable($zip_store)) {
					...
					if ($v_list == 0) {
                        $error = true;
                        $this->log(
                            esc_html__('Create zip error', 'tvr-microthemer'),
                            '<p>' . esc_html__('Error : ', 'tvr-microthemer') . $archive->errorInfo(true).'</p>'
                        );
					}
					else {
                        $this->log(
                            esc_html__('Zip package created', 'tvr-microthemer'),
                            '<p>' . esc_html__('Zip package successfully created.', 'tvr-microthemer') .
							'<a href="'.$this->thispluginurl.'zip-exports/'.$dir_name.'.zip">' .
							esc_html__('Download zip file', 'tvr-microthemer') . '</a>
						</p>',
                            'notice'
                        );
					}
                    ...
                    $error = true;
                    $this->log(
                        esc_html__('Zip store error', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('The directory %s is not writable. ', 'tvr-microthemer'),
			...
			function read_meta_file($meta_file) {
				...
				if (is_file($meta_file)) {
					...
					if ( is_readable($meta_file) )  {
                        ...
                        $this->log(
                            esc_html__('Read meta.txt error', 'tvr-microthemer'),
							'<p>' . esc_html__('WordPress does not have permission to read: ', 'tvr-microthemer') .
			...
			function read_readme_file($readme_file) {
				...
				if (is_file($readme_file)) {
					...
					if ( is_readable($readme_file) )  {
                        ...
                        $this->log(
                            esc_html__('Read readme.txt error', 'tvr-microthemer'),
							'<p>' . esc_html__('WordPress does not have permission to read: ', 'tvr-microthemer'),
			...
			function tvr_delete_micro_theme($dir_name) {
				...
				if (is_array($this->file_structure[$dir_name])) {
					foreach ($this->file_structure[$dir_name] as $dir => $file) {
						if (!unlink($this->micro_root_dir . $dir_name.'/'.$file)) {
                            $this->log(
                                esc_html__('File delete error', 'tvr-microthemer'),
                                '<p>' . esc_html__('Unable to delete: ', 'tvr-microthemer') .
                                $this->root_rel($this->micro_root_dir .
                                    $dir_name.'/'.$file) . '</p>'
                            );
                            $error = true;
				...
				if ($error != true) {
                    $this->log(
						...
						'<p>' . sprintf(
							esc_html__('All files within %s were successfully deleted.', 'tvr-microthemer'),
					...
					if (!rmdir($this->micro_root_dir . $dir_name)) {
                        $this->log(
                            esc_html__('Delete directory error', 'tvr-microthemer'),
							'<p>' . sprintf(
								esc_html__('The empty directory: %s could not be deleted.', 'tvr-microthemer'),
								$this->readable_name($dir_name)
							) . '</p>'
                        );
                        $error = true;
                        ...
                        $this->log(
                            esc_html__('Directory successfully deleted', 'tvr-microthemer'),
                            '<p>' . sprintf(
								esc_html__('%s was successfully deleted.', 'tvr-microthemer'),
								$this->readable_name($dir_name)
							) . '</p>',
                            'notice'
                        );
			...
			function update_meta_file($meta_file) {
                ...
                if (isset($_POST['prev_micro_name']) and ($_POST['prev_micro_name'] != $_POST['theme_meta']['Name'])) {
                    ...
                    if (is_writable($orig_name)) {
                        if (rename($orig_name, $new_name)) {
                            ...
                            if ($this->savePreferences($pref_array)) {
                                $this->log(
                                    esc_html__('Design pack renamed', 'tvr-microthemer'),
                                    '<p>' . esc_html__('The design pack directory was successfully renamed on the server.', 'tvr-microthemer') . '</p>',
                                    'notice'
                                );
                            }
                        }
                            ...
                            $this->log(
                                esc_html__('Directory rename error', 'tvr-microthemer'),
								'<p>' . sprintf(
									esc_html__('The directory, %s could not be renamed for some reason.', 'tvr-microthemer'),
                        ...
                        $this->log(
                            esc_html__('Directory rename error', 'tvr-microthemer'),
							'<p>' . sprintf(
								esc_html__('WordPress does not have permission to rename the directory %1$s to match your new theme name "%2$s". ', 'tvr-microthemer'),
								$this->root_rel($orig_name),
				...
				if (!file_exists($meta_file)) {
					if (!$write_file = fopen($meta_file, 'w')) {
                        $this->log(
                            sprintf( esc_html__('Create %s error', 'tvr-microthemer'), 'meta.txt' ),
                            '<p>' . sprintf(esc_html__('WordPress does not have permission to create: %s', 'tvr-microthemer'), $this->root_rel($meta_file) . '. '.$this->permissionshelp ) . '</p>'
                        );
					}
					else {
						fclose($write_file);
...
*/';
                    ...
                    $this->log(
                        'meta.txt '.$task,
                        '<p>' . sprintf( esc_html__('The %1$s file for the design pack was %2$s', 'tvr-microthemer'), 'meta.txt', $task ) . '</p>',
                        'dev-notice'
                    );
				}
				else {
                    $this->log(
                        sprintf( esc_html__('Write %s error', 'tvr-microthemer'), 'meta.txt'),
						'<p>' . esc_html__('WordPress does not have "write" permission for: ', 'tvr-microthemer') .
			...
			function update_readme_file($readme_file) {
				...
				if (!file_exists($readme_file)) {
					if (!$write_file = fopen($readme_file, 'w')) {
                        $this->log(
                        	sprintf( esc_html__('Create %s error', 'tvr-microthemer'), 'readme.txt'),
							'<p>' . sprintf(
							esc_html__('WordPress does not have permission to create: %s', 'tvr-microthemer'),
				...
				if ( is_writable($readme_file) )  {
					$data = stripslashes($_POST['tvr_theme_readme']); // don't use striptags so html code can be added
                     ...
                     $this->log(
                        'readme.txt '.$task,
						'<p>' . sprintf(
							esc_html__('The %1$s file for the design pack was %2$s', 'tvr-microthemer'),
							'readme.txt', $task
						) . '</p>',
                        'dev-notice'
                     );
                    ...
                    $this->log(
                        sprintf( esc_html__('Write %s error', 'tvr-microthemer'), 'readme.txt'),
						'<p>' . esc_html__('WordPress does not have "write" permission for: ', 'tvr-microthemer') .
					...
					if ($this->is_acceptable($file)) {
						...
						if (is_writeable($dest_dir) ) {
							...
							and copy($_FILES['upload_file']['tmp_name'], $dest_dir . $file)) {
                                $this->log(
                                    esc_html__('File successfully uploaded', 'tvr-microthemer'),
									'<p>' . sprintf(
                                ...
                                if ($this->is_screenshot($file)) {
                                    ...
                                    if (!$final_dimensions = $this->wp_resize(
                                        ...
                                        $thumbnail)) {
                                        $this->log(
                                            esc_html__('Screenshot thumbnail error', 'tvr-microthemer'),
											'<p>' .
											sprintf(
												wp_kses(
													__('Could not resize <b>%s</b> to thumbnail proportions.', 'tvr-microthemer'),
													array( 'b' => array() )),
												$root_rel_thumb
											) . $img_full_path .
											esc_html__(' thumb: ', 'tvr-microthemer') .$thumbnail.'</p>'
                                        );
                                    }
                                    else {
                                        // update the file_structure array
                                        ...
                                        $this->file_structure[$this->preferences['theme_in_focus']][$file] = $file;
                                        $this->log(
                                            esc_html__('Screenshot thumbnail successfully created', 'tvr-microthemer'),
											'<p>' . sprintf(
												esc_html__('<b>%s</b> was successfully created.', 'tvr-microthemer'),
												$root_rel_thumb
											) . '</p>',
                                            'notice'
                                        );
						...
						// it's not writable
                            ...
                            $this->log(
                                esc_html__('Write to directory error', 'tvr-microthemer'),
								'<p>'. esc_html__('WordPress does not have "Write" permission to the directory: ', 'tvr-microthemer') .
                        ...
                        $this->log(
                            esc_html__('Invalid file type', 'tvr-microthemer'),
                            '<p>' . esc_html__('You have uploaded a file type that is not allowed.', 'tvr-microthemer') . '</p>'
                        );
            ...
            function log_file_upload_error($error){
                switch ($error) {
                    case 1:
                        $this->log(
                            esc_html__('File upload limit reached', 'tvr-microthemer'),
                            '<p>' . esc_html__('The file you uploaded exceeded your "upload_max_filesize" limit. This is a PHP setting on your server.', 'tvr-microthemer') . '</p>'
                        );
                        break;
                    case 2:
                        $this->log(
                            esc_html__('File size too big', 'tvr-microthemer'),
                            '<p>' . esc_html__('The file you uploaded exceeded your "max_file_size" limit. This is a PHP setting on your server.', 'tvr-microthemer') . '</p>'
                        );
                        break;
                    case 3:
                        $this->log(
                            esc_html__('Partial upload', 'tvr-microthemer'),
                            '<p>' . esc_html__('The file you uploaded only partially uploaded.', 'tvr-microthemer') . '</p>'
                        );
                        break;
                    case 4:
                        $this->log(
                            esc_html__('No file uploaded', 'tvr-microthemer'),
                            '<p>' . esc_html__('No file was detected for upload.', 'tvr-microthemer') . '</p>'
                        );
                        break;
                }
            }
			...
			function resize($img, $max_width, $max_height, $newfilename) {
				...
				if (!extension_loaded('gd') && !extension_loaded('gd2')) {
                    $this->log(
                        esc_html__('GD not loaded', 'tvr-microthemer'),
                        '<p>' . esc_html__('The PHP extension GD is not loaded.', 'tvr-microthemer') . '</p>'
                    );
					return false;
				}
				//Get Image size info
				...
				switch ($imgInfo[2]) {
					...
					default:
                        $this->log(
                            esc_html__('File type error', 'tvr-microthemer'),
                            '<p>' . esc_html__('Unsuported file type. Are you sure you uploaded an image?', 'tvr-microthemer') . '</p>'
                        );
				...
				if (($max_width > $width) or ($max_height > $height)) {
                    $this->log(
                        esc_html__('Dimensions too big', 'tvr-microthemer'),
						'<p>' . sprintf(
							esc_html__('The resize dimensions you specified (%1$s x %2$s) are bigger than the original image (%3$s x %4$s). This is not allowed.', 'tvr-microthemer'),
				...
				switch ($imgInfo[2]) {
					...
					default:
                    $this->log(
                        esc_html__('Image resize failed', 'tvr-microthemer'),
                        '<p>' . esc_html__('Your image could not be resized.', 'tvr-microthemer') . '</p>'
                    );
                    return false;
                    break;
				}
